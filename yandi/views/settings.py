...
def option_view():
    if request.method == "GET":
        ...
        config_info = {
            ...
            "github_user": config_data['github_user'],
            "github_password": config_data['github_password'],
            "github_page": config_data['github_page'],
            "mail_user_dict": '\n'.join(config_data['mail_user_dict']),
            "mail_password_dict": '\n'.join(config_data['mail_password_dict']),
        ...
        if request.form.get("source") == "thread_settings":
            ...
            if connectiondb(config_db).update_one({'config_name': config_name}, {"$set": update_config}):
                ...
                return "Warning"
        # update thread config
        if request.form.get("source") == "github_settings":
            update_config = {
                "github_user": request.form.get('github_user'),
                "github_password": request.form.get('github_password'),
                "github_page": int(request.form.get('github_page')),
            }
            if connectiondb(config_db).update_one({'config_name': config_name}, {"$set": update_config}):
                return "success"
            else:
        ...
        elif request.form.get("source") == "mail-user-password":
            update_config = {
                "mail_user_dict": request.form.get('username_list').split('\n'),
                "mail_password_dict": request.form.get('password_list').split('\n'),
            }
            if connectiondb(config_db).update_one({'config_name': config_name}, {"$set": update_config}):
                return "success"
            else:
